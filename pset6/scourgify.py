"""
    Program expects user to provide two command-line arguments
    converts input to output, splitting each name into a first and
    last name
"""
import csv
import sys
...
def main():
    ...
    with open(sys.argv[2], "w") as file:
        ...
        writer.writeheader()
        for row in output:
            writer.writerow({"first": row["first"], 
                             "last": row["last"], 
                             "house": row["house"]})
